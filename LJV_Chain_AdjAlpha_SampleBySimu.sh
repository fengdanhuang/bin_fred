...
awk -v NOfFiles=$NOfFiles -v Step_Block=$Step_Block -v NOfSteps=$NOfSteps -v temperature=$temperature -v nAlpha=$nAlpha '{	
	if ($2=="candidate_alpha"){
		...
		refOver_Sum[i] += $2;
		refOver_Sqr[i] += $3;
		tarOver_Sum[i] += ($4/1000);
		tarOver_Sqr[i] += ($4/1000) * ($4/1000) ; #average by new block average, so old block average square will be useless.
	...
	tarBlockCount = NOfFiles;   #Each file is a simulation, each simulation is taken as a block.
