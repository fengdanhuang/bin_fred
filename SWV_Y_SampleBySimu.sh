...
Y_Value=`grep "Y_Value =" $myfiles | head -n 1 | awk '{print $4}'`
...
awk -v NOP=$NOP -v NPAIRS=$NPAIRS -v NOfCor=$NOfCor -v temperature=$temperature -v HSB=$HSB -v Y_Value=$Y_Value '{
	if ( $1=="#_of_blocks" ){
		...
		refS = ref_Err*ref_Err*1*(1-1) + ref_Avg*ref_Avg*1;
		totSRef += refS;
		totSumRef += ref_Avg * 1;
		...
		refSo = refOver_Err*refOver_Err*1*(1-1) + refOver_Avg*refOver_Avg*1;
		totSORef += refSo;
		totSumORef += refOver_Avg * 1;
	}else if ( $1=="reference" && $3=="correlation" ){
		refCor = $NF;
		s12Ref = 1*(refCor*sqrt((refS/1 - ref_Avg^2)*(refSo/1-refOver_Avg^2))+ref_Avg*refOver_Avg);
		totS12Ref += s12Ref;
		
	}else if ( $1=="Time" && $3=="ref" ){
		time_Ref += $10
	}else if ( $1=="target" && $2=="system" && $3=="average" ){
#		printf "tarTotalBlocksIII = %d\n", tarTotalBlocks;
		tarTotalBlocks += 1;
...
#		printf "%d,%20.15e\n", i, tar_Avg[i];
		...
		tarS[i] = tar_Err[i]*tar_Err[i]*1*(1-1) + tar_Avg[i]*tar_Avg[i]*1; #Note: tar_Err[] do not work, because it times 0.
#		printf "------%e %e %e\n", tar_Avg[i], tar_Err[i], tarS[i];

		totSTar[i] += tarS[i];
		totSumTar[i] += tar_Avg[i] * 1;
		i++;
		if (i== NPAIRS+1){
			readAvg = 0;
			...
			k=0;
                	for (m=0; m<NPAIRS; m++){
                        	for (n=m+1; n<=NPAIRS; n++){
                                	totSumTarIJ[k] += tar_Avg[m]*tar_Avg[n];
                                	k++;
                        	}
                	}
		}
		...
		tarSo = tarOver_Err*tarOver_Err*1*(1-1) + tarOver_Avg*tarOver_Avg*1;
		totSOTar += tarSo;
		...
		tarCor[j] = $1;
		s12Tar[j] = 1*(tarCor[j]*sqrt((tarS[j]/1 - tar_Avg[j]^2)*(tarSo/1-tarOver_Avg^2))+tar_Avg[j]*tarOver_Avg);
...
#		printf "---%e\n", totSTar[i];
	...
	for (i=0; i<=NPAIRS; i++){	
		...
		a[i] = B[i] / (Y_Value^i);
		aErr[i] = BErr[i] / (Y_Value^i);
#		printf "	%3d   %20.15e %10.3e %5.1e (CPU)\n", i, B[i], BErr[i], totErr[i];
		printf "        %3d   %20.15e %10.3e %5.1e (CPU)\n", i, a[i], aErr[i], totErr[i];
#		printf "        %20.15e %10.3e %5.1e \n", B[i], BErr[i], totErr[i] > "ValueErr_B"NOP"_BySimulation.txt";
		printf "	%20.15e %10.3e %5.1e \n", a[i], aErr[i], totErr[i] > "ValueErr_B"NOP"_BySimulation.txt";
#		printf "        %20.15e %10.3e %5.1e \n", B[i], BErr[i], totErr[i];
	}

	printf "\n	Correlation between coefficients (Sampling by simulation)= \n";
	k = 0;
	for (i=0; i<NPAIRS; i++)
...
#			printf "tarTotalBlocks=%d  totSumTarIJ[k]=%25.15e totSumTarI=%25.15e totSumTarJ=%25.15e\n", tarTotalBlocks, totSumTarIJ[k], totSumTar[i], totSumTar[j];
#			printf "tarTotalBlocks=%d  totSTar[i]=%25.15e totSTar[j]=%25.15e\n", tarTotalBlocks, totSTar[i], totSTar[j];
#			printf "covariance=%e\n", covariance;
#			printf "varianceI=%e\n", varianceI;
#			printf "varianceJ=%e\n", varianceJ;
			...
			printf "	      %25.15e\n", corBB[k]> "Correlation_B"NOP"_BySimulation.txt";
			k++;
		}
