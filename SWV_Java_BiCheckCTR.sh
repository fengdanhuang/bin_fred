...
chainFrac=${chainFrac%?}
treeFrac=`grep "treeFrac =" $myfiles | head -n 1 | awk '{print $6}'`
treeFrac=${treeFrac%?}
ringFrac=`grep "ringFrac =" $myfiles | head -n 1 | awk '{print $9}'`
...
awk -v NOP=$NOP -v NPAIRS=$NPAIRS -v NOfCor=$NOfCor -v Y_Value=$Y_Value -v NOP=$NOP -v lamda=$lamda -v chainFrac=$chainFrac -v treeFrac=$treeFrac -v ringFrac=$ringFrac 'BEGIN{
	x = 1;
        for (i=2; i<=NOP; i++) x *= i;
        HSB = (1-NOP) / x;
}{
...
#		totS12Tar[j] += s12Tar[j];
	...
	printf "	*****Calculate virial coefficients B%d *****\n", NOP;
...
#		printf "Y_Value=%d, i=%d, Y_Value^i=%d", Y_Value, i, Y_Value^i;
		a[i] = totSum[i] / (Y_Value^i);
		aErr[i] = totErr[i] / (Y_Value^i);		
		printf "	%3d   %20.15e %10.3e %5.1e (CPU)\n", i, a[i]*HSB, aErr[i]*sqrt(HSB^2), relatTotErr[i];
		printf "	%20.15e %10.3e %5.1e \n", a[i]*HSB, aErr[i]*sqrt(HSB^2), relatTotErr[i] > "ValueErr_B"NOP"_l"lamda".txt";

#		printf "	%3d   %20.10e\n", i, aErr[i]*sqrt(HSB^2) * sqrt(totalTime) > "Difficulty_B"NOP"_c"chainFrac"_t"treeFrac"_r"ringFrac".txt";
		printf "	%3d   %20.10e\n", i, aErr[i]*sqrt(HSB^2) * sqrt(totalTime) > "Difficulty_B"NOP"_l"lamda".txt";

#		if (i == 2){	
#			printf "     %.1f	%10.3e\n", chainFrac, aErr[i]*sqrt(HSB^2) > "B"NOP"_Uncertainty_c"chainFrac"_t"treeFrac"_r"ringFrac".txt";	
#		}
	}
	...
	for (i=0; i<NPAIRS; i++)
		for (j=i+1; j<=NPAIRS; j++){
			...
			varianceI = totS[i] - totSum[i]*totSum[i];
			varianceJ = totS[j] - totSum[j]*totSum[j];	
...
#                       printf "varianceJ=%e\n", varianceJ;
			...
			printf "	      %25.15e\n", corBB[k]> "Correlation_B"NOP"_l"lamda".txt";
			k++;
		}
